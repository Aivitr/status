...
function QualityChartInner({
  ...
  const coverages = useMemo(
    () => points.map((p) => p.coverage).filter((c): c is number => typeof c === 'number'),
    [points],
  );
  const bundles = useMemo(
    () => points.map((p) => p.bundleKb).filter((b): b is number => typeof b === 'number'),
    [points],
  );
  ...
  const yCoverage = useMemo(
    () =>
      scaleLinear<number>({
        domain: [Math.max(0, Math.floor((minCov - 10) / 10) * 10), 100],
        range: [innerH, 0],
        nice: true,
      }),
    [minCov, innerH],
          ...
          {COV_MIN >= yCoverage.domain()[0] && (
            <g>
              <line
                x1={0}
                x2={innerW}
                y1={yCoverage(COV_MIN)}
                y2={yCoverage(COV_MIN)}
            ...
            {points.map((p) => {
              ...
              return (
                <g key={`dot-${p.id}`}>
                  ...
                  {typeof p.bundleKb === 'number' && (
                    <circle
                      cx={cx}
                      cy={yBundle(p.bundleKb)}
                      r={3}
                      fill="var(--panel-surface)"
                      stroke="var(--accent)"
                      strokeWidth={2}
          ...
          <div className="mt-0.5 flex items-center gap-2.5 tabular-nums text-[var(--text-secondary)]">
            {typeof hovered.coverage === 'number' && (
              <span className="text-[var(--status-success)]">
                CI Pass: {hovered.coverage.toFixed(1)}%
              </span>
            )}
            {typeof hovered.bundleKb === 'number' && (
              <span className="text-[var(--accent)]">
                Churn: {hovered.bundleKb >= 0 ? `+${hovered.bundleKb}` : hovered.bundleKb}
              </span>
            )}
          </div>
        </div>
...
export function QualityBenchmarksChart({
  qualityBenchmarks,
  isLoading = false,
  className,
}: QualityBenchmarksChartProps) {
  const coverageHistory = useMemo(
    () => qualityBenchmarks?.coverageHistory ?? [],
    [qualityBenchmarks?.coverageHistory],
  );
  const bundleHistory = useMemo(
    () => qualityBenchmarks?.bundleHistory ?? [],
    [qualityBenchmarks?.bundleHistory],
  );
  ...
  const points = useMemo<ChartPoint[]>(() => {
    ...
    return Array.from({ length: len }, (_, i) => {
      ...
      return {
        ...
        bundleKb: bun?.bundleKb,
      };
    });
  }, [coverageHistory, bundleHistory]);

  const currentCoverage =
    qualityBenchmarks?.currentCoveragePct ??
    (coverageHistory.length > 0 ? coverageHistory[coverageHistory.length - 1].coverage : undefined);
  const currentBundle =
    qualityBenchmarks?.currentBundleKb ??
    (bundleHistory.length > 0 ? bundleHistory[bundleHistory.length - 1].bundleKb : undefined);
        ...
        {hasData && (
          <div className="flex items-center gap-2.5 font-mono text-[10px] tabular-nums text-[var(--text-secondary)]">
            <div className="flex items-center gap-1">
              ...
              <span>
                CI Pass: {currentCoverage !== undefined ? `${currentCoverage.toFixed(1)}%` : '--'}
              </span>
            </div>
            <div className="flex items-center gap-1">
        ...
        {isLoading && !hasData ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-1 text-center">
            ...
            <span className="font-mono text-[10px] text-[var(--text-muted)]">
              Awaiting test coverage & bundle telemetry
            </span>
          </div>
        ) : (
          <ParentSize debounceTime={10}>
            {({ width, height }) =>
              width > 0 && height > 0 ? (
                <QualityChartInner width={width} height={height} points={points} />
              ) : null
            }
          </ParentSize>
        )}
