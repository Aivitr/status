...
export function GitBranchGraph({
  ...
  const rawBranches = useMemo(() => gitBranchGraph?.branches ?? [], [gitBranchGraph?.branches]);
  const rawNodes = useMemo(() => gitBranchGraph?.nodes ?? [], [gitBranchGraph?.nodes]);
  ...
  const entries: GitLogEntry<CustomCommitMeta>[] = useMemo(() => {
    ...
    return rawNodes.map((node) => {
      const email = node.author.includes('@') ? node.author : `${node.author}@github.com`;
      return {
        hash: node.sha,
      ...
      return (
        ...
        >
          <div className="flex items-center gap-1.5 border-b border-[var(--panel-border-subtle)] pb-1 mb-1.5 font-mono text-[9px]">
            ...
            <span className="ml-auto font-bold text-[var(--text-primary)]">#{shortSha}</span>
          </div>
      ...
      <div className="z-10 flex shrink-0 flex-col border-b border-[var(--panel-border-subtle)] bg-[var(--panel-surface)]">
        <div className="flex items-center justify-between px-3.5 py-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-semibold text-[var(--text-primary)]">Git Branch Network</h3>
          </div>
          {hasData && (
