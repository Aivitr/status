...
export default function Home() {
  ...
  const { data: telemetry, isLoading, error } = useTelemetry(selectedProjectId);

  const isFailed = telemetry?.vitalPulse.latestWorkflow.status === 'FAILED';
  const systemStatus = error
    ? 'CHANNEL DEGRADED // OFFLINE CACHE'
    : isFailed
      ? 'SYSTEM DEGRADED // CI FAILED'
      : 'ALL SYSTEMS NOMINAL';

  return (
    <DashboardShell statusProps={{ systemStatus }}>
      <ProjectCardStream />
      <KpiMatrix telemetry={telemetry} isLoading={isLoading} />
      <section
